...
object AudioHelper {
    ...
    fun playBell(
        ...
        Thread {
            val streamType = if (useAlarmStream) AudioManager.STREAM_ALARM else AudioManager.STREAM_MUSIC
            var remaining = executions
            while (remaining > 0) {
                try {
                    val toneGen = ToneGenerator(streamType, volume)
                    toneGen.startTone(ToneGenerator.TONE_CDMA_HIGH_L, 800)
                    Thread.sleep(800L + gapMs)
                    toneGen.stopTone()
                    toneGen.release()
                } catch (e: Exception) {
                    e.printStackTrace()
